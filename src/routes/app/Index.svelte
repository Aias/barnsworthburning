<script lang="ts">
	...
	import { displayTitle, type IndexEntry } from '#lib/records.js';
...
<ol class="index" class:themed={true} {...restProps}>
	{#each index as entry (entry.id)}
		<li class="index-entry">
			<Link class="entity-link" record={entry} active={entry.id === currentId}>
				{displayTitle(entry)}
	...
	<li class="index-entry">
		<Link href="/feed.xml" data-sveltekit-preload-data="false">RSS Feed</Link>
	</li>
	<li class="index-entry">
