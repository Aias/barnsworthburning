...
- **Hosting moves off Cloudflare Workers** to the mac mini (SvelteKit `adapter-bun`, Bun toolchain matching rcr's), managed by pm2 with an auto-deploy script mirroring rcr's, exposed publicly through a Cloudflare Tunnel. Postgres stays tailnet-only.
...
1. **Publish `@aias/hozo` 0.4.0 to npm.** A versioned package keeps the Mac mini independent of adjacent worktrees. Hozo builds `dist/` via `prepack`, which runs on `bun publish`.
