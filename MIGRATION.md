...
- **Data access is direct Drizzle queries** using `@aias/hozo` schema + relations with the `postgres-js` driver. The internal `/api/*` REST layer is deleted; pages load data in idiomatic SvelteKit `load` functions. No view-model translation layer — components consume record/link shapes natively.
...
3. **Read-only Postgres role** for bwb with SELECT on `records`, `links`, `media` only (SQL provided for Nick to run — the agent never runs migrations or grants).
